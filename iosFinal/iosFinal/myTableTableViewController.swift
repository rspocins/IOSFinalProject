...
class myTableTableViewController: UITableViewController {
    ...
    */
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let s: song = mySongs[indexPath.row]
        let iD = s.type.rawValue

        let cell = tableView.dequeueReusableCell(withIdentifier: iD, for: indexPath)

        // Configure the cell...
        let string = mySongs[indexPath.row].name
        let sub = mySongs[indexPath.row].artist
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let detailViewController = segue.destination as? DetailViewController {
            if let indexPath = self.tableView.indexPathForSelectedRow{
                detailViewController.s  = mySongs[indexPath.row]
            }
        }
    }
}
